// Decompiled by AS3 Sorcerer 1.40
// http://www.as3sorcerer.com/

//com.company.assembleegameclient.map._1wN

package com.company.assembleegameclient.map {
import _11j._1ST;

import _GI.ConfettiEffect;
import _GI._08I;
import _GI._69;

import com.company.assembleegameclient.objects.BasicObject;
import com.company.assembleegameclient.objects.GameObject;

import flash.display.IGraphicsData;
import flash.events.Event;
import flash.utils.Dictionary;
import flash.utils.getTimer;

public class _1wN extends Map {

    public static const _07X:int = 1;
    public static const _1SG:Number = 16;

    public static function _0o8(_arg1:Number):Number {
        return ((_arg1 / _1SG));
    }

    public function _1wN(_arg1:int = -1) {
        this._uX = new Vector.<BasicObject>();
        this._0El = new Vector.<int>();
        this.graphicsData_ = new Vector.<IGraphicsData>();
        super(null);
        if (_arg1 == _07X) {
            addEventListener(Event.ENTER_FRAME, this._1bv);
        }
        ;
    }
    private var _LB:Boolean = false;
    private var _uX:Vector.<BasicObject>;
    private var _0El:Vector.<int>;
    private var _05:uint = 0;
    private var _1eJ:uint = 0;
    private var time:uint = 0;
    private var graphicsData_:Vector.<IGraphicsData>;

    override public function addObj(_arg1:BasicObject, _arg2:Number, _arg3:Number):void {
        _arg1.x_ = _arg2;
        _arg1.y_ = _arg3;
        if (this._LB) {
            this._uX.push(_arg1);
        } else {
            this.internalAddObj(_arg1);
        }
        ;
    }

    override public function internalAddObj(_arg1:BasicObject):void {
        var _local2:Dictionary = _1NM;
        if (_local2[_arg1.objectId_] != null) {
            return;
        }
        ;
        _arg1.map_ = this;
        _local2[_arg1.objectId_] = _arg1;
    }

    override public function internalRemoveObj(_arg1:int):void {
        var _local2:Dictionary = _1NM;
        var _local3:BasicObject = _local2[_arg1];
        if (_local3 == null) {
            return;
        }
        ;
        _local3.removeFromMap();
        delete _local2[_arg1];
    }

    override public function update(_arg1:int, _arg2:int):void {
        var _local3:BasicObject;
        var _local4:int;
        this._LB = true;
        for each (_local3 in _1NM) {
            if (!_local3.update(_arg1, _arg2)) {
                this._0El.push(_local3.objectId_);
            }
            ;
        }
        ;
        this._LB = false;
        for each (_local3 in this._uX) {
            this.internalAddObj(_local3);
        }
        ;
        this._uX.length = 0;
        for each (_local4 in this._0El) {
            this.internalRemoveObj(_local4);
        }
        ;
        this._0El.length = 0;
    }

    override public function draw(_arg1:_18q, _arg2:int):void {
        var _local3:BasicObject;
        this.graphicsData_.length = 0;
        var _local4:int;
        for each (_local3 in _1NM) {
            _local4++;
            _local3._0Wa(_1SG);
            _local3.draw(this.graphicsData_, _arg1, _arg2);
        }
        ;
        graphics.clear();
        if (this.graphicsData_.length > 0) {
            graphics.drawGraphicsData(this.graphicsData_);
        }
        ;
    }

    private function _07Y(_arg1:Number, _arg2:Number, _arg3:int = 20, _arg4:int = 12447231):void {
        var _local5:GameObject = new GameObject(null);
        _local5.x_ = _0o8(_arg1);
        _local5.y_ = _0o8(_arg2);
        var _local6:_69 = new _69(_local5, _arg3, _arg4);
        this.addObj(_local6, _local5.x_, _local5.y_);
    }

    private function _1ab(_arg1:Number, _arg2:Number, _arg3:int = 20, _arg4:int = 12447231):void {
        var _local5:_1ST = new _1ST();
        var _local6:_1ST = new _1ST();
        _local5.x_ = _0o8(_arg1);
        _local5.y_ = _0o8(_arg2);
        _local6.x_ = _0o8(_arg1);
        _local6.y_ = _0o8(600);
        var _local7:ConfettiEffect = new ConfettiEffect(_local5, _local6, _arg4, _arg3, true);
        this.addObj(_local7, _local5.x_, _local5.y_);
    }

    private function _0Rj(_arg1:Number, _arg2:Number, _arg3:Number, _arg4:Number, _arg5:int = 200, _arg6:int = 12447231):void {
        var _local7:GameObject = new GameObject(null);
        _local7.x_ = _0o8(_arg1);
        _local7.y_ = _0o8(_arg2);
        var _local8:_1ST = new _1ST();
        _local8.x_ = _0o8(_arg3);
        _local8.y_ = _0o8(_arg4);
        var _local9:_08I = new _08I(_local7, _local8, _arg6, _arg5);
        this.addObj(_local9, _local7.x_, _local7.y_);
    }

    private function _1bv(_arg1:Event):void {
        var _local2 = 600;
        var _local3 = 600;
        if (this.time != 0) {
            this._05 = (getTimer() - this.time);
        }
        ;
        this._1eJ = (this._1eJ + this._05);
        this.time = getTimer();
        if (this._1eJ > 500) {
            this._1eJ = 0;
            this._1ab((Math.random() * 600), -100);
        }
        ;
        this.update(this.time, this._05);
        this.draw(null, this.time);
    }


}
}//package com.company.assembleegameclient.map

